...
class Grid_World:
    ...
    def __init__(self, rows = 4, cols = 5, obs = 5) -> None:
        self.rows = rows
        self.cols = cols
        ...
        if obs > 0:
            temp = [(i, j) for i in range(0, rows) for j in range(0, cols)]
            temp = temp[1:-2]
            self.obs_loc = [temp[i] for i in sample(range(0, (rows - 1) * (cols - 1)), obs)]
